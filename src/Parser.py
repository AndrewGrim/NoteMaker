...
def parse(tokens: List[LexerToken], html: str, css: str) -> None:
	...
	while i < len(tokens):
		...
		if t.id == MD.HEADING:
			...
			htmlTags.append(f'<div class="string">{t.content}</div>')
		elif t.id == MD.CODEBLOCK_TYPE:
			htmlTags.append(f'<div class="type">{t.content}</div>')
		elif t.id == MD.CODEBLOCK_FLOW:
			htmlTags.append(f'<div class="flow">{t.content}</div>')
		elif t.id == MD.CODEBLOCK_DIGIT:
			htmlTags.append(f'<div class="digit">{t.content}</div>')
